...
import com.example.zt_flutter_sample_v2.R
import com.example.zt_flutter_sample_v2.databinding.LayoutComputeBinding
...
class ComputeLayoutPlatform(
    ...
    var bind: LayoutComputeBinding? = null
    var viewModel: CountBean? = null

    companion object {
        // Android原生View 在Flutter引擎上注册的唯一标识，在Flutter端使用时必须一样
        private const val ANDROID_SEND_FLUTTER_DATA_NOTICE: String =
            "androidSendFlutterDataNotice" // Android端 向 Flutter端 发送数据
        private const val ANDROID_GET_FLUTTER_DATA_NOTICE: String =
            "androidGetFlutterDataNotice" // Android端 获取 Flutter端 数据
        private const val FLUTTER_SEND_ANDROID_DATA_NOTICE: String =
            "flutterSendAndroidDataNotice" // Flutter端 向 Android端 发送数据
        private const val FLUTTER_GET_ANDROID_DATA_NOTICE: String =
    ...
    private fun initData(rootContext: Context, args: Any?) {
        ...
        viewModel?.curNum?.observe(owner, object : Observer<Int> {
            override fun onChanged(value: Int) {
                bind?.tvCount?.setText("点击次数:" + value)
            }
        })
        viewModel?.flutterNum?.observe(owner, object : Observer<Int> {
            override fun onChanged(value: Int) {
                bind?.tvSaveCount?.setText("接收Flutter端发送的点击次数:" + value)
            }
        })
        viewModel?.getFlutterNum?.observe(owner, object : Observer<Int> {
            override fun onChanged(value: Int) {
                bind?.tvGetCount?.setText("获取Flutter页面点击次数:" + value)
            }
        })
    }
